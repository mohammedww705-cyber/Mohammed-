    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...connected.map((c) => _CloudTile(cloud: c, onToggle: () => setState(() => c['connected'] = false))),
          const SizedBox(height: 20),
          const Text('غير متصل', style: TextStyle(color: AppColors.text2, fontSize: 13, fontWeight: FontWeight.w500)),
          const SizedBox(height: 10),
          ...notConnected.map((c) => _CloudTile(cloud: c, onToggle: () => _connect(c))),
        ]),
      ),
    );
  }

  void _connect(Map<String, dynamic> cloud) {
    showDialog(context: context, builder: (_) => AlertDialog(
      backgroundColor: AppColors.surface2,
      title: Text('ربط ${cloud['name']}', style: const TextStyle(color: AppColors.text1)),
      content: const Text('سيتم ربط حسابك.', style: TextStyle(color: AppColors.text2)),
      actions: [
      ...
      child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: connected ? AppColors.accent2.withOpacity(0.15) : AppColors.accent.withOpacity(0.15),
              borderRadius: BorderRadius.circular(8),
            ),
            child: Text(connected ? 'متصل' : 'ربط',
                style: TextStyle(color: connected ? AppColors.accent2 : AppColors.accent, fontSize: 11, fontWeight: FontWeight.w500)),
          ),
        ),
      ]),
    );
