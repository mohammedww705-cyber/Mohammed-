    ...
    return SafeArea(
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          ...[
            {'name': 'تقرير الربع الرابع.pdf', 'size': '2.3 MB', 'icon': Icons.picture_as_pdf_rounded, 'color': Color(0xFFFF6B6B), 'bg': AppColors.cardRed},
            {'name': 'مشروع_النهائي.zip', 'size': '48 MB', 'icon': Icons.folder_zip_rounded, 'color': Color(0xFFEF9F27), 'bg': AppColors.cardAmber},
            {'name': 'فيديو الاجتماع.mp4', 'size': '312 MB', 'icon': Icons.video_file_rounded, 'color': Color(0xFF378ADD), 'bg': AppColors.cardBlue},
            {'name': 'خطة العمل.docx', 'size': '890 KB', 'icon': Icons.description_rounded, 'color': AppColors.accent, 'bg': AppColors.cardPurple},
          ].map((f) => Container(
...
class _CatCard extends StatelessWidget {
  ...
  final Color color;
  final Color bg;
  const _CatCard({required this.icon, required this.label, required this.color, required this.bg});
